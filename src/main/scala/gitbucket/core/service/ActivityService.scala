...
trait ActivityService {
  ...
  def getRecentActivities(): DBIO[Seq[Activity]] =
    Activities
      .join(Repositories).on((t1, t2) => t1.byRepository(t2.userName, t2.repositoryName))
  ...
  def getRecentActivitiesByOwners(owners : Set[String]): DBIO[Seq[Activity]] =
    Activities
      .join(Repositories).on((t1, t2) => t1.byRepository(t2.userName, t2.repositoryName))
  ...
  def recordForkActivity(userName: String, repositoryName: String, activityUserName: String, forkedUserName: String): DBIO[Int] =
    Activities += Activity(userName, repositoryName, activityUserName,
      "fork",
