...
trait ProtectedBranchService {
  ...
  private def getProtectedBranchInfoOpt(owner: String, repository: String, branch: String)
                                       (implicit e: ExecutionContext): DBIO[Option[ProtectedBranchInfo]] =
    ProtectedBranches
      ...
      .map { _.
        ...
        map{ case (t1, contexts) =>
          ProtectedBranchInfo(t1.userName, t1.repositoryName, true, contexts, t1.statusCheckAdmin)
        }.headOption
      }

  def getProtectedBranchInfo(owner: String, repository: String, branch: String)
                            (implicit e: ExecutionContext): DBIO[ProtectedBranchInfo] =
    getProtectedBranchInfoOpt(owner, repository, branch).map(_.getOrElse(ProtectedBranchInfo.disabled(owner, repository)))

  def getProtectedBranchList(owner: String, repository: String): DBIO[Seq[String]] =
    ProtectedBranches.filter(_.byRepository(owner, repository)).map(_.branch).result

  def enableBranchProtection(owner: String, repository: String, branch:String, includeAdministrators: Boolean, contexts: Seq[String]): DBIO[Unit] = {
    disableBranchProtection(owner, repository, branch) >>
      (ProtectedBranches += ProtectedBranch(owner, repository, branch, includeAdministrators && contexts.nonEmpty)) >>
      DBIO.seq(
        contexts.map { context =>
          ProtectedBranchContexts += ProtectedBranchContext(owner, repository, branch, context)
        }: _*
      )
  }

  def disableBranchProtection(owner: String, repository: String, branch:String): DBIO[Int] =
